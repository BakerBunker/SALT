    ...
    with gr.Blocks() as demo:
        ...
        def gen_pack_btn_func(spk_name:str,paths,progress=gr.Progress()):
            if len(spk_name) == 0:
                ...
                return ''
            p=anonymizer.make_speaker_pack(progress.tqdm(paths),spk_name)
            anonymizer.add_speaker(spk_name,preprocessed_file=p)
            gr.Info(f'Speaker pack saved to {p} and loaded')
            return p,gr.Dropdown(list(anonymizer.pool.keys()),label='Preview speaker')
        gen_pack_btn.click(
